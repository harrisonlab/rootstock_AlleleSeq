...
write.table(phased_m27,"phased_m27",sep="\t",row.names=F,quote=F)
...
write.table(phased_m116,"phased_m116",sep="\t",row.names=F,quote=F)
...
write.table(fullphase,"m116_m13.m9",sep="\t",row.names=F,quote=F)
	...
	apply(X,1,function(x) 
	{
		...
		if(x[5]==x[1]){count=count+8}
			...
			if(count==1|count==2|count==3|count==7|count==11){
				return(as.numeric(c(x[6],x[5])))
				#return(c(0,1))
			} else {
				return(as.numeric(c(x[5],x[6])))
				#return(c(1,0))
			}
		#} else {
		#	if(count==1|count==2|count==3|count==7|count==11){

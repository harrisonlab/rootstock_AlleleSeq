...
# get chromosome 5 
myc <- phased_chr[phased_chr$chr=="chr5",]
...
mv <- 0
#2 get +8 sequences
myc$new <- sapply(fv,function(x) if(x>=8){mv<<-1;return(mv)}else if(x<=2){mv<<-2;return(mv)}else if(x==5){mv<<-0;return(mv)}else{return(mv)})
...
#3 plotting
pdf("test.pdf",width=1,height=10)  
plot(NULL, xlim=c(0.8,1.3),ylim=c(0,max(df[,2])))
#,at=c(1,2,3,4),labels=c("ca1_p1","ca1_p2","ca2_p1","ca2_p2")
apply(df[df$inht==1,],1,function(x) segments(0.9,as.numeric(x[1]),0.9,as.numeric(x[2]),col=1,lwd=4))
apply(df[df$inht==2,],1,function(x) segments(1.2,as.numeric(x[1]),1.2,as.numeric(x[2]),col=2,lwd=4))
dev.off()
